...
export function buildTaskPrompt(task: KanbanTask): string {
  ...
  return [
    ...
    `- If you notice other work outside this task, do not do it now. Queue it instead: curl -sS -X POST "\${MONITOR_API_URL:-http://127.0.0.1:5000}/api/agent/tasks" -H "Authorization: Bearer $MONITOR_AGENT_TOKEN" -H "Content-Type: application/json" -d '{"title":"...","repository":"${task.repository}","description":"..."}'`,
